...
def main(event, environment):
    ...
    if not event.get('branches'):
        ...
        branches = ['Scranton', 'Akron', 'Buffalo', 'Rochester', 'Syracuse', 'Utica',
                    'Binghamton', 'Albany', 'Nashua', 'Pittsfield', 'Stamford', 'Yonkers',
                    'New York']
    else:
    ...
    try:
        for branch in branches:
            ...
            response = requests.get(
                url=f'www.domain.com/branches/?branch={branch}'
            )
            response = response.json()
            for result in response.get('result'):
                if not ingestionCompleted(table, Key('branch_id').eq(str(result['branch_id'])), result, 'branch_'):
